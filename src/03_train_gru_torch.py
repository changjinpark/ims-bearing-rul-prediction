...
이 파일의 역할:
    PyTorch의 nn.GRU를 사용해서 RUL 예측 모델을 학습합니다.

왜 이 파일을 만들었나:
    - RandomForest/MLP는 한 시점의 feature 행만 보고 RUL을 예측합니다.
    - GRU는 최근 여러 시점의 feature 흐름을 함께 보고 RUL을 예측합니다.
    - 과제/블로그에서는 이 파일을 "딥러닝 라이브러리를 사용한 GRU 모델"로 설명하면 됩니다.
...
처음 보면 중요한 관점:
    ...
    - GRU는 그 sequence를 보고 마지막 시점의 RUL을 예측합니다.

전체 코드 흐름:
    1. feature CSV를 읽습니다.
    2. train/test 실험을 나눕니다.
    3. feature를 표준화합니다.
    4. 여러 행을 GRU 입력 sequence로 묶습니다.
...
def feature_columns(df: pd.DataFrame, target: str) -> list[str]:
    """
    4. GRU 입력으로 사용할 feature 컬럼만 고릅니다.
...
def plot_predictions(
    ...
    """17. test 데이터의 실제 RUL과 GRU 예측 RUL을 그래프로 저장합니다."""
    ...
    ax.plot(ordered_pred, label="gru_torch", linewidth=1.3, color="#1f77b4", alpha=0.9)
    ax.set_title(f"PyTorch GRU RUL prediction on test sequences ({target})")
...
def run(args: argparse.Namespace) -> None:
    ...
    여기서 실제로 CSV를 읽고, sequence를 만들고, GRU를 학습하고, 산출물을 저장합니다.
    """
    ...
    metrics_path = args.tables_dir / f"gru_torch_metrics_{name_suffix}.csv"
    ...
    predictions_path = args.tables_dir / f"gru_torch_predictions_{name_suffix}.csv"
    predictions_df = test_meta.copy()
    predictions_df["actual"] = y_test
    predictions_df["prediction_gru_torch"] = pred_test
    predictions_df["absolute_error"] = np.abs(predictions_df["actual"] - predictions_df["prediction_gru_torch"])
    predictions_df.to_csv(predictions_path, index=False)
    print(f"saved: {predictions_path}", flush=True)

    history_path = args.tables_dir / f"gru_torch_training_history_{name_suffix}.csv"
    artifacts.history.to_csv(history_path, index=False)
    print(f"saved: {history_path}", flush=True)

    prediction_plot_path = args.figures_dir / f"gru_torch_predictions_{name_suffix}.png"
    ...
    history_plot_path = args.figures_dir / f"gru_torch_training_loss_{name_suffix}.png"
    plot_training_history(artifacts.history, history_plot_path)
    print(f"saved: {history_plot_path}", flush=True)

    model_path = args.models_dir / f"gru_torch_model_{name_suffix}.pt"
    save_model(artifacts, scaler, cols, args, model_path)
    print(f"saved: {model_path}", flush=True)

    config_path = args.models_dir / f"gru_torch_config_{name_suffix}.json"
    ...
    print("\nFinal PyTorch GRU metrics:", flush=True)
...
def parse_args() -> argparse.Namespace:
    """21. 터미널 옵션을 정의합니다."""
    root = project_root()
    output_root = root / "artifacts" / "03_train_gru_torch" / "outputs_all"

    parser = argparse.ArgumentParser(description="Train a PyTorch GRU for IMS Bearing RUL.")
    parser.add_argument(
        "--features-csv",
        type=Path,
